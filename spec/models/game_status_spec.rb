...
RSpec.describe GameStatus, :type => :model do
  ...
  describe "#update method" do
    it "calls update player location method with args" do
      allow(GameStatus).to receive(:update_player_location).with(options)
      allow(GameStatus).to receive(:proximity_to_rabbit?)
      expect(GameStatus).to receive(:update_player_location).with(options)
