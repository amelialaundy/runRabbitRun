...
skip_before_action :verify_authenticity_token
	...
	def sanitise_input(chat_info)
	  ...
	  options = {}
	  options['displayName'] = CGI::escapeHTML(chat_info['nickname']).slice(0, 30)
	  options['text'] = CGI::escapeHTML(chat_info['text']).slice(0, 300)
	  options['email'] = CGI::escapeHTML(email).slice(0, 100)
